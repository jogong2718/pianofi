...
def get_queue_name(model: str, environment: str) -> str:
    ...
    queue_map = {
        ...
        ("basicpitch", "production"): "basicpitch_job_queue_prod",
        ("basicpitch", "development"): "basicpitch_job_queue_dev",
    }
...
def queue_job(
    ...
    4. Push job to appropriate Redis queue (amt or picogen or basicpitch)
    
    Args:
        job_id: ID of the job to queue
        file_key: File key for validation
        user_id: ID of the user (for permission check)
        model: Model to use ('amt' or 'picogen' or 'basicpitch')
